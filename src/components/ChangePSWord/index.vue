<template>
  <div id="changePSWord">
    <header id="header">
      <span class="back-wrap" @click="()=>{this.$router.push('/set')}">
        ...
        <div class="from-input">
          ...
          <div class="captcha">
            <mt-field v-model="smsCode" placeholder="输入短信验证码">
              <mt-button
                @click.prevent="setSmsCode()"
                v-show="show"
                v-preventReClick
                class="send-captcha"
              >{{smsCodeTest}}</mt-button>
              <span v-show="!show" class="send-captcha time-captcha">{{count}} s后重发</span>
            </mt-field>
          </div>
        </div>
        <div class="from-password">
          <mt-field label="新密码" placeholder="输入新密码" type="password" v-model="userPassword"></mt-field>
          <mt-field label="确认密码" placeholder="确认输入密码" type="password" v-model="conPassword"></mt-field>
        </div>
        <div class="from-btn">
          <mt-button type="danger" @click.prevent="update()" class="toast">确认修改</mt-button>
        </div>
      </form>
    </div>
  </div>
                ...
                if (
                  ...
                  if (this.userPassword == this.conPassword) {
                    //判断两次密码是否一致
										...
										this.$http.post("sms/updateUser",data)
										.then(res => {
											if (res.message == "修改成功") {
												Toast(res.message)
											}else{
