		...
		<div class="main">
			<form class="from-wrap">
				...
				<div class="from-password">
					<mt-field label="设置密码" placeholder="输入密码" type="password" v-model="userPassword"></mt-field>
					<mt-field label="确认密码" placeholder="确认输入密码" type="password" v-model="conPassword"></mt-field>
				</div>
				<div class="from-btn">
					<mt-button type="danger" @click.prevent="setRegister()" class="toast">注 册</mt-button>
				</div>
			</form>
		</div>
	</div>
			...
			setSmsCode() {
				if (this.userPhone) {
					...
					if (reg.test(this.userPhone)) {
						this.$api.sms.createSmsCode(this.userPhone).then(() => {
							const TIME_COUNT = 30;
							if (!this.timer) {
